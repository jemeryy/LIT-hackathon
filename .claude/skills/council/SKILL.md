...
name: council
model: opus
description: Use when someone asks to roast an idea, pressure-test or stress-test an idea, validate a business idea, "convene the council", get a brutal second opinion before building something, or says "/council". Spins up a 5-persona council that attacks the idea from every angle, then a Judge returns one GO / RESHAPE / KILL verdict with the cheapest test to de-risk it.
argument-hint: "[the idea to pressure-test]"
---

## What this does

Claude's default is to agree with you. `/council` is the opposite. It convenes a council of five independent persona agents who tear an idea apart and build it up from every angle, then a Judge synthesizes everything into one honest verdict. Use it before you sink time and money into building the wrong thing.
...
Spin up **all five agents in parallel in a single message** (one Task call each, `subagent_type: research-sonnet`). Paste the same brief into each, then give each its persona mandate below.
...
Before you write the verdict, run a quick **pre-mortem**: first state the real win-condition (what would have to be true for this to count as a success, in one line), then assume it is 12 months later and this idea has clearly failed, and name the single most likely cause of death. This is forward-looking and different from the Contrarian's attack: it forces you to pick the ONE thing that actually kills it, and the cheapest test below should target exactly that cause.
...
**Win-condition:** [what has to be true for this to count as a success]
**Pre-mortem:** [it's 12 months later and this failed — the single most likely cause]
