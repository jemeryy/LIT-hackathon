...
This skill is self-contained. It depends only on built-in Claude Code tools (the `Agent` tool with the `research-sonnet` agent, `Write`, and web search/fetch used inside those agents) plus `report-template.html` in this same folder. No external scripts, APIs, paid services, or other skills are required. Drop the folder into any `.claude/skills/` directory and it works.
...
Spawn **five `research-sonnet` agents in a single message** so they run concurrently. Each gets the SAME topic framing plus its own lens. Use these exact prompts, substituting `{TOPIC}` and a one-line `{TOPIC_FRAME}` (your Phase 0 interpretation):
...
**4b. Verify every citation (parallel agents).** Spawn `research-sonnet` agents in one message, one per distinct citation cluster (group related claims; ~4-6 agents). Each agent prompt:
...
- **Long sources go through NotebookLM.** When a lens (or you) needs to read a LONG source (a full PDF, a long video, a lengthy filing or report), route the reading through `.claude/skills/notebooklm-use/SKILL.md` (ask targeted questions) instead of pulling the full text into context. Phase 4 citation verification stays local and unchanged: NotebookLM answers are unverified, so the primary-source check still fetches the real source. If NotebookLM is unavailable, read locally as before.
